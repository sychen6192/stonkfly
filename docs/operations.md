...
To watch a run, `python -m stonkfly monitor` opens a page at `http://127.0.0.1:8765` for the most recently updated run under `runs/` (`--out` picks another, `--port` another port). It shows the chart image the network receives, each decision with the DNp20 rates and gate that produced it, the execution result or veto reason, account value, orders, and KC and dopamine-cell activity, refreshing every few seconds. It is read-only: ledgers open in SQLite read-only mode, nothing in the run directory is written, `.env` and key files are never read, and it listens on this computer only. It can run alongside a worker; stopping it does not affect the run.
